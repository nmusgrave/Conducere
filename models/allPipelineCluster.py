...
def execute(args):
  ...
  problem_file = getattr(library, "pipelineCluster")
  ...
  both = best_of_both_worlds(results)
  print
  print
...
def best_of_both_worlds(results):
  boths = {key: mean(result) - std_dev(result) for key, result in results.iteritems()}
